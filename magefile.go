...
// golib - a core library of Go packages.
package main
...
import (
	...
	projectSupport "github.com/svengreb/golib/internal/support/project"
...
const (
	...
	// goModuleVersionToolGoModUpgrade is the version for the "go-mod-upgrade" Go module command.
	goModuleVersionToolGoModUpgrade = "v0.8.0"
)
...
func init() {
	...
	elderWand, ewErr := elder.New(
		...
		elder.WithGoRunnerOptions(
			taskGo.WithRunnerEnv(envSliceToMap(os.Environ())),
		),
		elder.WithNib(ink),
	)
...
func UpgradeMods() {
	modVer, modVerErr := semver.NewVersion(goModuleVersionToolGoModUpgrade)
	if modVerErr != nil {
		ew.ExitPrintf(
			1,
			nib.ErrorVerbosity,
			"%s\n  ↳ %v", color.RedString("Parsing Go module version %q:", goModuleVersionToolGoModUpgrade),
			modVerErr,
		)
	}
	ew.Infof("Updating outdated Go dependencies")
	if err := ew.GoModUpgrade(taskGoModUpgrade.WithModuleVersion(modVer)); err != nil {
